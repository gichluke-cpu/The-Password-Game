...
def show_game_over_screen(reason):
    ...
    f = tk.Frame(root, bg="#f8d7da")
    f.pack(expand=True, fill="both")

    tk.Label(f, text="💀 GAME OVER 💀", font=("Georgia", 30, "bold"),
             bg="#f8d7da", fg="#721c24").pack(pady=40)

    tk.Label(f, text=reason, font=("Georgia", 14),
             bg="#f8d7da", fg="#721c24", wraplength=400).pack(pady=10)

    tk.Button(f, text="Play again", font=("Georgia", 14),
              command=show_game_screen, bg="#dc3545", fg="white").pack(pady=30)
    #tk.Button(f, text="Exit", font=("Georgia", 10),
              #command=show_exit_dialog, bg="#f4a261", fg="white").place(relx=1.0, #x=-10, y=10, anchor="ne")
    # <--- BẮT ĐẦU THÊM NÚT NHẠC (GÓC TRÊN PHẢI, DƯỚI EXIT) ---
    music_frame = tk.Frame(f, bg="#f8d7da") # Sử dụng màu nền của màn hình Game Over
    # Đặt frame ở góc phải, dưới nút Exit (y=40)
    music_frame.place(relx=1.0, x=-10, y=10, anchor="ne") 
