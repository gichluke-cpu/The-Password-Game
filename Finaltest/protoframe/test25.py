...
def burningsound():
    ...
    pygame.mixer.music.play(loops=0) 
def batsound():
    pygame.mixer.music.load("C:\WebScrapingLibrary_NeoM2\cus-im-batman.mp3")
        #pygame.mixer.music.set_volume(0.7) # Đặt âm lượng (0.0 đến 1.0)
        
        # SỬA: loops=-1 để lặp lại vô hạn
    pygame.mixer.music.play(loops=0) 
def snoresound():
    pygame.mixer.music.load("snoring.mp3")
        #pygame.mixer.music.set_volume(0.7) # Đặt âm lượng (0.0 đến 1.0)
...
def show_batman_sleeping():
    stupid_win = tk.Toplevel(root)
    stupid_win.title("BATMAN IS ASLEEP")
    stupid_win.geometry("600x400")
    stupid_win.resizable(False, False)
...
def show_game_screen(initial_password=""):
    ...
    # Batman and Pie buttons horizontal
    btn_frame = tk.Frame(game_frame, bg="#fdf8e4")
    btn_frame.pack(pady=10)

    tk.Button(btn_frame, text="Gọi Batman 🦇", bg="#b0c4de",
              font=("Georgia", 10, "bold"), command=summon_batman).pack(side="left", padx=5)

    tk.Button(btn_frame, text=f"Thêm Bánh {PIE_EMOJI}",
              command=add_pie_emoji, bg="#ffe4b5",
              font=("Georgia", 10, "bold")).pack(side="left", padx=5)
